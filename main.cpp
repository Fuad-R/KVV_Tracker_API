...
std::optional<StopRecord> parseStopRecord(const json& stop) {
    ...
    auto localId = getJsonString(stop, {"localid", "localId", "local_id"});
    if (!localId && stop.contains("properties") && stop.at("properties").is_object()) {
        localId = getJsonString(stop.at("properties"), {"stopId", "stopID", "localid", "localId", "local_id"});
    }
...
void ensureStopsInDatabase(const json& searchResult, const std::string& originalSearch) {
    ...
    const char* insertSql =
        "INSERT INTO stops (stop_id, localid, stop_name, city, mot, location, original_search) "
        "VALUES ($1, $2, $3, $4, $5, ST_SetSRID(ST_MakePoint($6, $7), 4326)::geography, $8) "
        "ON CONFLICT (stop_id) DO UPDATE SET "
        "localid = COALESCE(EXCLUDED.localid, stops.localid), "
